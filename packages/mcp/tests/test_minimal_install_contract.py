...
def test_optional_profiles_follow_the_runtime_import_boundaries() -> None:
    ...
    assert {"documents", "embedding", "vector", "dense", "all"} <= set(domain)
    assert {"documents", "images", "embedding", "vector", "dense", "all"} <= set(mcp)
    assert "ocr" not in domain
    assert "ocr" not in mcp
    ...
    assert any("sagasmith-core[documents]" in item for item in _normalized(mcp["documents"]))
    assert any("sagasmith-core[documents]" in item for item in _normalized(mcp["images"]))
    assert any("sagasmith-core[embedding]" in item for item in _normalized(mcp["embedding"]))
    assert any("sagasmith-core[vector]" in item for item in _normalized(mcp["vector"]))
...
def test_minimal_text_mcp_starts_without_importing_heavy_capabilities(tmp_path: Path) -> None:
    script = textwrap.dedent(
        ...
        os.environ["SAGASMITH_COC_MCP_HOME"] = str(home)
